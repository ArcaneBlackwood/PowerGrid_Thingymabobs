...
import dev.thingymabobs.util.ThingymabobsMath.TransformerEquivalent;
...
public final class TransformerComponent extends OrientableComponent implements IComponentGoggleInformation {
    ...
    public static final IntProperty TOTAL_TURNS = new IntProperty(
            ...
            30,
            2,
            ThingymabobsMath.TRANSFORMER_MAX_TOTAL_TURNS
    );
    ...
    public static final IntProperty PRIMARY_TURNS = new IntProperty(
            ...
            25,
            1,
            ThingymabobsMath.TRANSFORMER_MAX_TOTAL_TURNS - 1
    );

    public static final CalculatedProperty<Integer> SECONDARY_TURNS = new CalculatedProperty<>(
            Thingymabobs.MOD_ID,
            "transformer_secondary_turns",
            placed -> ThingymabobsMath.secondaryTurns(placed.get(PRIMARY_TURNS), placed.get(TOTAL_TURNS)),
            Object::toString
    );
    ...
    public static final CalculatedProperty<Float> RATIO = new CalculatedProperty<>(
            Thingymabobs.MOD_ID,
            "transformer_ratio",
            placed -> ThingymabobsMath.transformerRatio(placed.get(PRIMARY_TURNS), placed.get(TOTAL_TURNS)),
    ...
    public void bake(
        ...
        TransformerEquivalent equivalent = ThingymabobsMath.transformerEquivalent(
        ...
        builder.couple(
                equivalent.ratio(),
                ThingymabobsMath.MIN_RESISTANCE,
                primaryInternal,
                primary2,
                secondaryInternal,
                secondary2
    ...
    public boolean tick(@NotNull PlacedComponent placed) {
        int total = ThingymabobsMath.totalTurns(placed.get(TOTAL_TURNS));
        int effectivePrimary = ThingymabobsMath.primaryTurns(placed.get(PRIMARY_TURNS), total);
    ...
    public boolean addToGoggleTooltip(
        ...
        int total = ThingymabobsMath.totalTurns(placed.get(TOTAL_TURNS));
        int primary = ThingymabobsMath.primaryTurns(placed.get(PRIMARY_TURNS), total);
        int secondary = ThingymabobsMath.secondaryTurns(primary, total);
        float ratio = ThingymabobsMath.transformerRatio(primary, total);
        ...
        tooltip.add(Component.translatable("thingymabobs.tooltip.transformer.rating", Math.round(THERMAL_RATING_WATTS)));
